# Package an ML Project as Installable Python Package
...
1. Update the `pyproject.toml` with the following contents:
...
2. Move into `fraud-detection` directory and run the python build command:
    ...
    > It should a `dist` directory and print successul result.

3. Check the `dist` directory and make sure `fraud-detection-0.1.0-*.whl` wheel package is existing.
