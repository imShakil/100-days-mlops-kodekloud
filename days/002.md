# Set Up and Configure Jupyter Notebook Server
...
1. Update `jupyter_lab_config.py`:
    ...
    - replaced `ServerApp.port` from `8000` to `8888`
    - replaced `ServerApp.ip` from `1.1.1.1` to `0.0.0.0`

2. Create the required directory if doesn't exists

    ```bash
    mkdir -p /root/notebooks
    ```

3. Start JupyterLab with the given commands:
...
4. Open the Jupyter UI from top right option
